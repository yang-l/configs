...
- Do not overthink the work. Take the simplest approach that meets the request. This covers a plan, an answer, and a change. Add no step, phase, abstraction, or contingency that the request does not need.
- Advisor gate — the main thread owns this gate. The rule that routes reviews to the `reviewer` agent does not apply here. Trigger (a): call `advisor` before every `ExitPlanMode`, including a simple plan. Fix every blocker. Then run the Codex review only when the plan meets the Codex trigger below. Trigger (b): in auto-mode only, call `advisor` before a consequential decision. A consequential decision is an architecture choice, a security-relevant change, or a multi-agent team structure. Fix every blocker, then call `advisor` again. Repeat until `advisor` agrees with no blocker left. Trigger (b) never fires in plan mode. Codex never runs under trigger (b), so trigger (b) has no fixed round limit. If a blocker stands after about 3 rounds, put the disagreement to the user instead of looping again.
- Codex trigger — run the Codex review only when the plan meets any one of these four conditions. The plan spans 2 or more domains. The plan touches 3 or more files. The plan changes architecture, security, data shape, or a public interface. The plan is team-tier or workflow-tier, as the Delegation section defines them. Skip the Codex review for every other plan.
...
- Codex prompt — give Codex the plan text and every decision the user already settled. State this contract in the prompt. Codex reports only Critical and High findings. Codex omits Medium and Low findings. Codex does not re-open a settled decision. Codex ends with a final line of exactly `VERDICT: approve` or `VERDICT: needs-attention`.
- Round limit — run two rounds, then stop. In round 1, fix every Critical and High finding. Skip a finding that re-opens a settled decision, or that primary evidence refutes. In round 2, run the review once more. Never run a third round. The `advisor` call in trigger (a) shares this two-round limit. The limit does not apply to trigger (b).
- Open questions — after the rounds finish, list only the open questions for the user to decide. Include every unresolved advisor blocker. Add `(codex)` to each question that a Codex round left unanswered. Put the list last in the plan, or last in the reply when no plan exists. Do not describe or list anything you fixed, resolved, or refuted. Do not claim agreement that a reviewer did not give. Delete any earlier sign-off line. Prepend one line that matches what happened. The line always carries the advisor slot.
...
- Do not add an inline comment that only restates the code. Add a comment only when it carries what the code cannot show. That means a non-obvious constraint, a breaking change, a known-bug workaround, a safety warning, or the reason for a surprising choice. Match the comment density of the surrounding file.
