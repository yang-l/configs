...
Spawn Opus agents for: architecture decisions, cross-cutting design, complex reasoning, review of any kind (code, security, plan), and large ingests (10k+ words); use fable for the hardest reasoning and long-horizon agentic tasks. All review goes to the `reviewer` agent — the `/code-review`, `/review`, and `/security-review` skills are the quick path for a small diff. Do not spawn a generic Opus agent — use a defined agent.
...
- Within an agent team or workflow, the same advisor triggers apply to each agent individually (stuck, approaching commitment, consequential decisions). Additionally, unless the whole formation is opus/fable or higher, add the `reviewer` agent as a final synthesis step at its default opus tier — not the sonnet tier used for an in-formation `reviewer` — escalating to `model: fable` for the highest-stakes work. The orchestrator acts on the verdict — revise and re-task agents on any blockers — before finalizing.
